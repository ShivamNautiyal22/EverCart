  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 leading-relaxed tracking-wider my-10">
      {filteredProducts.map((product) => (
        <div key={product.id} className="relative rounded-[10px] overflow-hidden group py-5 border-2 border-gray-400 px-2">
          <img src={product.image} alt={product.title + product.id} className="w-full h-[300px] mix-blend-multiply" />
          <div className="mt-3">
            ...
            <div className="flex flex-col gap-2 mt-3">
              <button className="bg-pink-200 cursor-pointer rounded-[5px] py-2 px-1 uppercase font-semibold">
                Add to Cart
              </button>
              <button className="bg-pink-200 cursor-pointer rounded-[5px] py-2 px-1 uppercase font-semibold">
                Buy Now
