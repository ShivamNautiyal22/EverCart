...
const CartProducts = ({ item, increaseCount, decreaseCount, removeItem }) => {
  return (
    <div className="flex items-center p-2 leading-relaxed tracking-wider border-2  rounded-xl">
      ...
      <div className="flex items-center justify-between w-full px-5">
        <div>
          ...
          <p className="line-clamp-1 pr-4">{item.title}</p>
        ...
        <button onClick={removeItem} className="bg-black text-white py-1 px-3 rounded-sm cursor-pointer">Remove</button>
      </div>
    </div>
  );
};
